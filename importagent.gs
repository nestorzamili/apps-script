function saveAgentManualInput(sheet, row) {
  ...
  const key = `${merchant}@${monthNum}@${Utilities.formatDate(
    dateObj,
    'GMT+8',
    'yyyy-MM-dd',
  )}`;
  ...
  const data =
    lastRow >= 2 ? storage.getRange(2, 1, lastRow - 1, 4).getValues() : [];
  let found = false;
  ...
  for (let i = 0; i < data.length; i++) {
    if (String(data[i][0]).trim() === key) {
      storage
        .getRange(i + 2, 2, 1, 3)
        .setValues([[rateFPX, rateEwallet, withdrawal]]);
      found = true;
      break;
    }
  }
...
function updateSingleRow(sheet, row) {
  ...
  const date = Utilities.formatDate(
    new Date(sheet.getRange(row, 1).getValue()),
    'GMT+8',
    'yyyy-MM-dd',
  );
  ...
  const manual =
    storeRowIndex >= 0 ? storeData[storeRowIndex + 1] : [key, 0, 0, 0];
  ...
  for (let i = 1; i < depData.length; i++) {
    ...
    const trxDate = Utilities.formatDate(
      new Date(rowD[idx['Transaction Date']]),
      'GMT+8',
      'yyyy-MM-dd',
    );
    const settleDate = Utilities.formatDate(
      new Date(rowD[idx['Settlement Date']]),
      'GMT+8',
      'yyyy-MM-dd',
    );
...
function importAgent() {
  ...
  if (lastRowWithData >= 5) {
    ledger.getRange(5, 1, lastRowWithData - 4, 12).clearContent();
  }

  // Get previous month closing balance
  const prevMonthClosingBalance = getPreviousMonthAgentClosingBalance(
    merchant,
    monthNum,
    year,
  );
  ...
  let cumulativeBalance = prevMonthClosingBalance;
  const results = [];
  for (let d = 1; d <= lastDay; d++) {
    const dateStr = Utilities.formatDate(
      new Date(year, monthNum - 1, d),
      'GMT+8',
      'yyyy-MM-dd',
    );
...
function getPreviousMonthAgentClosingBalance(merchant, currentMonth, year) {
  const prevMonthNum = currentMonth === 1 ? 12 : currentMonth - 1;
  const prevYear = currentMonth === 1 ? year - 1 : year;
  return calculateMonthAgentClosingBalance(merchant, prevMonthNum, prevYear);
}

function calculateMonthAgentClosingBalance(merchant, month, year) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const deposit = ss.getSheetByName('Deposit');
  const storage = ss.getSheetByName('AgentStorage');

  if (!deposit || !storage) return 0;
  ...
  if (month > 1 || year > 2025) {
    cumulativeBalance = getPreviousMonthAgentClosingBalance(
      merchant,
      month,
      year,
    );
